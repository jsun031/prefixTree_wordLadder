...
class Solution {
public:
	//construct all shortest paths from beginWord to endWord via wordList
	vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
		if (wordList.empty()) {
			return vector<vector<string>>();
		}
		Trie dict;
		Trie data;//for string map
		bool exist = false;
		//Perhaps this is not the most concise way to get the right answer, as we can add both begin and end to data and dict.
		//But excluding begin and limiting end in data only could save us half time.
		for (int i = 0; i < wordList.size(); i++) {
			if (wordList.at(i) == endWord) {
				data.addWord(wordList.at(i));
				exist = true;
				continue;
			}
			if (wordList.at(i) != beginWord) {
				data.addWord(wordList.at(i));
				dict.addWord(wordList.at(i));
			}
		}
		//if endWord does not exist in the wordList, the would be no root
		if (!exist) {
			return vector<vector<string>>();
		}
		//Use bfs to search the word path
		Trie* next = new Trie();
		next->addWord(beginWord);
		Trie* cur = new Trie();
		int counter = 1;//indicate how many steps we have gone

		Trie* nextEnd = new Trie();//This is needed for optimization, as we choose the small one between next and nextEnd for next iteration
		nextEnd->addWord(endWord);
		bool order = true;//indicate if the path is from begin to end.

		bool oneMore = true;//indacate there is no valid path found in current iteration
		vector<vector<string>> ans;//answer
		while (!next->empty() && oneMore) {
			delete cur;
			cur = next;
			next = new Trie();

			counter++;
			Trie toDelete;//delete must be done at the end of a iteration in order to find all the valid string map. Therefore, it is necessay to remember them.
			while (!cur->empty()) {	
				string s = cur->front();
				cur->removeWord(s);
				vector<string> ending = nextEnd->findNeighbor(s);
				//If nextEnd distance from s is only 1, the path is reached this round. Therefore, no further iteration.
				if (!ending.empty()) {
					oneMore = false;
					//We would try to extend both ends to get the complete paths. 
					//make sure begin is at back, so we need reverse it only once
					if (order) {
						for (auto it : ending) {
							ans.push_back(vector<string>{it, s});
						}
					} else {
						for (auto it : ending) {
							ans.push_back(vector<string>{s, it});
						}
					}
				} else {
					vector<string> neighbor = dict.findNeighbor(s);
					data.markNeighbor(s, neighbor);
					for (int i = 0; i < neighbor.size(); i++) {
						toDelete.addWord(neighbor.at(i));
						next->addWord(neighbor.at(i));
					}
				}				
			}
			while (!toDelete.empty()) {
				dict.removeWord(toDelete.front());
				toDelete.removeWord(toDelete.front());
			}
			if (oneMore && next->size() > nextEnd->size()) {
				swap(next, nextEnd);
				order = !order;
			}
		}
		...
		vector<vector<string>> refer;
		//extend the end instead of beginning
		while (!ans.empty() && ans[0].back().compare(beginWord) != 0) {
			refer.clear();
			for (int j = 0; j < ans.size(); j++) {
				vector<string> nextString = data.mapWord(ans[j].back());
				for (int k = 0; k < nextString.size(); k++) {
					vector<string> current = ans[j];
					current.push_back(nextString[k]);
					refer.push_back(current);
				}
			}
			swap(refer, ans);
		}
		...
		while (!ans.empty() && ans[0].back().compare(endWord) != 0) {
			refer.clear();
			for (int j = 0; j < ans.size(); j++) {
				vector<string> nextString = data.mapWord(ans[j].back());
				for (int k = 0; k < nextString.size(); k++) {
